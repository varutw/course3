...
activityLabel<-scan("activity_labels.txt",what=character())
activityName<-activityLabel[seq(2,length(activityLabel),2)]
...
colnames(dataComplete)<-c(extractMeanStd,"subject","activity")

#switch subject and activity columns to the front
dataComplete<-subset(dataComplete,select=c("subject","activity",extractMeanStd))

#change data in activity column from number to the name of each activity
for (i in 1:6){
  dataComplete$activity<-gsub(i,activityName[i],dataComplete$activity)
}
